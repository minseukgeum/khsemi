<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.ArrayList, buy.model.vo.Buy, admin.model.vo.PageInfo" %>
<%
...
<body>
	...
	<div class="outer">
			...
			<table id="listArea">
				...
				<%		for(Buy b : list){ %>
							<tr>
								...
								<td><%= b.getP_id() %></td>
								<td><%= b.getOrder_name() %></td>
								<td><%= b.getOrder_address() %></td>
		...
		<div class="pagingArea" align="center">
			...
			<!-- 맨 처음으로 -->
			<button onclick="location.href='<%= request.getContextPath() %>/main.ad?currentPage=1'">&lt;&lt;</button>
		
			<!-- 이전 페이지로 -->
			<button onclick="location.href='<%= request.getContextPath() %>/main.ad?currentPage=<%= currentPage - 1 %>'"
				id="beforeBtn">&lt;</button>
			<script>
			...
			<% for(int p = startPage; p <= endPage; p++){ %>
				...
				<% } else{ %>
					<button id="numBtn" 
						onclick="location.href='<%= request.getContextPath() %>/main.ad?currentPage=<%= p %>'">
						<%= p %>
					</button>
				<% } %>
			<% } %>
			...
			<!-- 맨 끝으로 -->
			<button onclick="location.href='<%= request.getContextPath() %>/main.ad?currentPage=<%= maxPage %>'">&gt;&gt;</button>
		</div>
