...
/**
 * Servlet implementation class MemberRecipeInsertServlet
 */
@WebServlet("/insert.th")
public class MemberRecipeInsertServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MemberRecipeInsertServlet() {
        super();
...
//		String title = request.getParameter("title");
		...
		// lib에 cos.jar 를 추가하여 진행 <== 파일도 받고 다른 일반 값들도 받아주는 역할
		
		// encType이 multipart/form-data로 전송되었는지 확인 (true면 실행)
...
//			System.out.println(originFiles);
			...
			int result = new BoardService().insertMemberRecipe(b, fileList);
			...
			if(result > 0) {
				...
				request.setAttribute("msg", "회원 레시피 등록에 실패하였습니다.");
